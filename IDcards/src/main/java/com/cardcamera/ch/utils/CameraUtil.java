...
public class CameraUtil {
    ...
    public void camera(Activity activity) {
        ...
        activity.startActivityForResult(intent, Constants.CameraActivity.REQUEST_CODE.CAMERA);
    }
     ...
     *
     * @param list
     * @param size      例如在去pictureSize时，需要判断是否可取到与preview一样的尺寸，如果不可以的话，则取相同比例的
     * @param minHeight
     * @return
     */
    public Size getPropSizeForHeight(List<Size> list, Camera.Size size, int minHeight) {
        ...
        //取预览的比例
        String ratio = LibUtils.showDecimalValue(size.width * 1d / size.height);
     ...
     *
     * @param list
     * @param minHeight 返回高度最接近 minHeight 的size
     * @return
     */
    public Size getPropSizeForHeight(List<Size> list, int minHeight) {
        return getPropSizeForHeight(list, minHeight, false);
    }
     ...
     * @param list
     * @param minHeight  返回高度最接近 minHeight 的size
     * @param getMaxIfNo 如果没有大于最小值的，是否取最大值
     * @return
     */
    public Size getPropSizeForHeight(List<Size> list, int minHeight, boolean getMaxIfNo) {
        Collections.sort(list, new CameraAscendSizeComparatorForHeight());
